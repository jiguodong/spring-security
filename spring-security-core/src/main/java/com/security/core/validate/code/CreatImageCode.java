...
@Getter
@Setter
public class CreatImageCode implements ValidateCreatImageCode {
    @Autowired
    ...
    public ImageCode CreatImageCode(ServletWebRequest request) {
        ...
        String sRand = "";
        for (int i = 0; i < ServletRequestUtils.getIntParameter(request.getRequest(),"length",securityProperties.getValidateCode().getImageCode().getLength()); i++) {
        ...
        g.dispose();
        return new ImageCode(image, sRand,ServletRequestUtils.getIntParameter(request.getRequest(),"expireIn",securityProperties.getValidateCode().getImageCode().getExpireIn()) );
    }
